class Solution {
public:
    bool search(int i, int j, int n, int m, vector<vector<char>>& board, int k,
                string word) {
        if (k == word.size())
            return true; // If we have matched all characters in the word,
                         // return true
        if (i == n || i < 0 || j < 0 || j == m || board[i][j] != word[k]) {
            return false; // If current cell is out of bounds or does not match
                          // the current character in word, return false
        }

        char ch =
            board[i]
                 [j]; // Store the current character before exploring neighbors
        board[i][j] =
        ...
        return op1 || op2 || op3 ||
               op4; // Return true if any of the recursive calls return true
    }
    bool exist(vector<vector<char>>& board, string word) {
        ...
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < m; j++) {
                // Check if the word can be formed starting from the current
                // cell
